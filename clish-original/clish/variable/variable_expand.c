 ...
 */
static char *
find_viewid_var(const char *viewid,
                const char *name)
{
    char      *result=NULL;
    regex_t    regex;
 ...
 */
static char *
escape_special_chars(const char *string,
...
/*--------------------------------------------------------- */
static char *
context_retrieve(const context_t *this,
                 const char      *name)
{
    char       *result       = NULL;
    const char *tmp          = NULL;
    ...
    if(this && this->pargv)
    {
        const ucli_parg_t *parg = ucli_pargv_find_arg(this->pargv,name);
        if(NULL != parg)
        {
            /* substitute the command line value */
            tmp = ucli_parg__get_value(parg);
        }
    }
    ...
    if(this && this->cmd)
    {
        ...
        escape_chars = ucli_command__get_escape_chars(this->cmd);
    }
 ...
 */
static char *
context_nextsegment(const context_t  *this,
                    const char      **string)
{
    const char *p      = *string;
    char       *result = NULL;
