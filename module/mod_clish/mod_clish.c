/* ucli_module.c --
 * 
 *       This module provides the supporting hooks for the ucli executable.
 */
...
usage(const char *filename)
{
    ...
    printf("               If undefined then '/etc/ucli;~/.ucli' will be used.\n");
}
 ...
 */
bool_t
ucli_access_callback(const ucli_shell_t *shell,
                      const char          *access)
{
...
/* ucli_script_callback.c --
 *
 *       Callback hook to action a shell script.
 */
bool_t
ucli_script_callback(const ucli_shell_t *shell,
                      const char          *script)
{
#ifdef DEBUG
    printf("CLI-EXEC: %s\n",script);
